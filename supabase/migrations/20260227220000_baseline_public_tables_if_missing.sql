/*
  IMPORTANTE: in Supabase SQL Editor incolla TUTTO IL CONTENUTO di questo file
  (Ctrl+A qui, Ctrl+C, poi incolla nella query). NON incollare il percorso del file.

  Baseline: create all public tables expected by the frontend if missing.
  Use this if you see "Could not find the table 'public.agencies' in the schema cache"
  (e.g. migrations were never applied, or you're on a fresh Supabase project).

  Tables: agencies, profiles, athletes, contracts, conflicts, deals, activities,
  chat_threads, chat_messages, notifications, campaigns, campaign_deliverables,
  reports, clauses.

  All CREATE TABLE use IF NOT EXISTS. Extra columns from later migrations
...
-- Clauses (extracted/normalized contract clauses; frontend may use for display)
...
-- Drop policies if they exist (by name), then create (avoids duplicate policy errors when re-running)
...
-- Realtime (ignore errors if already in publication)
DO $$
BEGIN
